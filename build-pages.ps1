$header = Get-Content -Path "partials\header.html" -Raw
$footer = Get-Content -Path "partials\footer.html" -Raw
$head = @"
<!DOCTYPE html>
<html lang="en-AU">
<head>
...
$main = @"
  ...
  <main>
    ...
    <div class="page-content">
      <p>Content for this page will go here. Quick Funding helps Australian businesses access finance from 90+ lenders.</p>
...
foreach ($p in $pages) {
    $title = $p[0]; $file = $p[1]
    $html = ($head -replace "\{\{TITLE\}\}", $title) + $header + ($main -replace "\{\{TITLE\}\}", $title) + $footer + $tail
    Set-Content -Path $file -Value $html -Encoding UTF8
    Write-Host "Wrote $file"
}
Write-Host "Done. Generated $($pages.Count) pages."
